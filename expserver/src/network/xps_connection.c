...
void connection_loop_close_handler(void *ptr)
{
    ...
    logger(LOG_DEBUG, "connection_loop_close_handler()", "connection closed");
}
...
void connection_source_handler(void *ptr)
{
    ...
    size_t remaining = DEFAULT_PIPE_BUFF_THRESH - connection->source->pipe->buff_list->len;
    size_t to_read = remaining < buff->size ? remaining : buff->size;

    int read_n = recv(connection->sock_fd, buff->data, to_read, 0);
    buff->len = read_n;
...
void connection_source_close_handler(void *ptr)
{
    ...
    {
        /*close connection*/
        logger(LOG_INFO, "connection_source_close_handler()", "both source and sink inactive, closing connection");
        connection_close(connection, false);
    }
}
...
void connection_sink_close_handler(void *ptr)
{
    ...
    if (!connection->source->active && !connection->sink->active)
    {
        /*close connection*/
        connection_close(connection, false);
        logger(LOG_INFO, "connection_sink_close_handler()", "both source and sink inactive, closing connection");
    }
}
